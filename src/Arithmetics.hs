...
evalArithmetics op (stk,dict) = case (op,stk) of
    ...
    ("div", Numbo n2 : Numbo n1 : rest) -> pure $ Right (Numbo (div n1 n2) : rest,dict) 

    ("div", Deci 0.0 : _ : _) -> pure $ Left (RunTime DivisionByZero)
    ("div", Deci f2 : Numbo n1 : rest) -> pure $ Right (Numbo (div n1 (truncate f2)) : rest,dict)
    
    ("div", Numbo 0 : _ : _) -> pure $ Left (RunTime DivisionByZero)
    ("div", Numbo n2 : Deci f1 : rest) -> pure $ Right (Numbo (div (truncate f1) n2) : rest,dict)

    ("div", Deci 0.0 : _ : _) -> pure $ Left (RunTime DivisionByZero)
    ("div", Deci f2 : Deci f1 : rest) -> 
        pure $ Right (Numbo (div (truncate f1) (truncate f2)) : rest,dict)
    ...
    ("==", b : a : rest) -> pure $ Right (Truthy (a == b) : rest,dict)
